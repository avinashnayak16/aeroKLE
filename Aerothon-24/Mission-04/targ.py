...
def send_ned_velocity(velocity_x, velocity_y, velocity_z):
    msg = vehicle.message_factory.set_position_target_local_ned_encode(
        ...
        mavutil.mavlink.MAV_FRAME_BODY_OFFSET_NED,  # frame (relative to drone body)
        0b0000111111000111,  # type_mask (only speeds enabled)
        0, 0, 0,  # x, y, z positions (not used)
        velocity_x, velocity_y, velocity_z,  # x (forward), y (right), z (down) velocities
...
def moveTotarget():
    ...
    clearFile("YelloWcoordinates.txt",320,410)
    time.sleep(1)    
