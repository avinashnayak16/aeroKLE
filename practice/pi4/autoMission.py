...
hotspotFlag = threading.Event()
hotspotFlag.set()
targetFlag = threading.Event()
targetFlag.set()
leftToRightFlag = threading.Event()
moveToaviFlag = threading.Event()
    ...
    try:
        while True:
            ...
            for i in range(len(scores)):
                if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)) and leftToRightFlag.is_set():
...
def getLatLon(x_pixel,y_pixel):
    ...
    # Dx = (np.tan(theta_x_rad) * vehicle.location.global_relative_frame.alt)
    # Dy = (np.tan(theta_y_rad) * vehicle.location.global_relative_frame.alt)
    Dx = (np.tan(theta_x_rad) * altitude)
    Dy = (np.tan(theta_y_rad) * altitude)
...
def loopCheckingThread():
    global is_new_hotspot,visited_hotspots,tarLat,tarLon
    print("loop checking thread is started")
    while True:
        prev_x, prev_y = None, None
        # if interruption_flag.is_set() and (not hotspotFlag.is_set() or not targetFlag.is_set() ):
        if interruption_flag.is_set():
            print("loop")
            print(countFile)
            x_pixel,y_pixel,hotspotNum,typeDetect = readingStepCoordinates(countFile-1)
            #print(x_pixel,y_pixel,hotspotNum,typeDetect)
            ...
            for i in range(3):
                ...
                print(x_pixel,y_pixel,prev_x, prev_y)
...
def go_to_location(latitude, longitude, altitude):
    ...
    while True:
        ...
        if interruption_flag.is_set() and moveToaviFlag.is_set():
...
def moveToavi():
    ...
    time.sleep(0.1)
    set_yaw(vehicle.heading)
    go_to_location(tarLat,tarLon,altitude)
    time.sleep(0.1)
    #visited_hotspots.append(tarLat,tarLon)
    # pic_thread = threading.Thread(target=take_picture,name="pic_image" ,args=(cap, lock))
    # pic_thread.start()
